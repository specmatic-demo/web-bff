...
const schemaPath = findFirstExistingPath([
    ...
    path.join(__dirname, '..', '.specmatic', 'repos', 'central-contract-repository', 'contracts', 'services', 'web-bff', 'graphql', 'schema.graphql'),
    path.join(__dirname, '..', '..', 'central-contract-repository', 'contracts', 'services', 'web-bff', 'graphql', 'schema.graphql')
]);
if (!schemaPath) {
...
function logDependencyError(dependency, endpoint, error, context = {}) {
    ...
    console.error(`[dependency-error] dependency=${dependency} endpoint=${endpoint} message="${message}" context=${JSON.stringify(context)}`);
}
console.log(`Dependency configuration: customer=${config.customerServiceBaseUrl}, catalog=${config.catalogServiceBaseUrl}, order=${config.orderServiceBaseUrl}, payment=${config.paymentServiceBaseUrl}, pricing=${config.pricingServiceAddress}, mqtt=${config.notificationBrokerUrl}`);
...
const rootValue = {
    ...
    orders: async ({ customerId, status, from, to }) => {
        const params = new URLSearchParams();
    ...
    placeOrder: async ({ input }) => {
        const customer = await httpJson(`${config.customerServiceBaseUrl}/customers/${encodeURIComponent(input.customerId)}`, { method: 'GET' });
    ...
    cancelOrder: async ({ orderId, reason }) => {
        ...
        const response = await httpJson(`${config.orderServiceBaseUrl}/orders/${encodeURIComponent(orderId)}/cancel`, {
            method: 'POST',
